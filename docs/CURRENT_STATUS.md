...
- 默认 Shell 使用克制的 ANSI 配色：路径和目录使用亮蓝色，进程状态、错误和后台 PID 分类显示。
- `BOOT_COLOR=0` 同时禁用内核与全部用户 ELF 的颜色，输出不含 ANSI 转义序列。
- `kill` 失败时报告目标 PID；刚执行完的 `ps` 已退出，或 PID 0、Shell 等受保护任务均不可终止。
