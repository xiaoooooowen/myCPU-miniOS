...
> 报告核对日期：2026 年 6 月 20 日  
> 核对口径：以当前仓库源码、构建脚本和实际自动化测试结果为准。

## 项目名称

基于自研 RV64I 模拟平台的 MiniOS 操作系统设计与实现
...
| 成员  | 学号  | 分工                              |
| --- | --- | ------------------------------- |
| 张晓文 | 20231071455 | 系统总体设计、MiniOS 内核、进程管理、文件系统、报告撰写 |


> 注：本项目以方案 A 为目标，重点实现一个可启动、可运行用户程序、具备进程管理、内存管理、文件系统与 Shell 交互能力的教学型操作系统。
...
1. **宿主环境层**：项目运行在 WSL / Linux 环境中，通过 CMake、Makefile 和 RISC-V 裸机工具链完成构建。
2. **硬件模拟层**：由自研 `cemu` 模拟器提供 RISC-V CPU、DRAM、CSR、MMU、Bus、UART、CLINT、PLIC 和 Block Device 等硬件抽象。
3. **MiniOS 内核层**：包括 Bootloader、内核启动、trap 处理、系统调用、物理页分配、虚拟内存、进程管理、调度器、文件系统和 ELF loader。
4. **用户程序层**：包括 C Shell、简化 libc、用户程序入口 `crt0.S`、`ls/cat/echo/pwd/ps/kill/mkdir/rm/touch/write` 等命令，以及 `fstest/forktest/argtest/spin` 等测试程序。

【图 1 占位：MiniOS / myCPU 总体架构图。建议展示 Host、cemu、Bootloader、MiniOS Kernel、MiniFS、User ELF、Shell 的层次关系。】
...
【图 2 占位：cemu 模拟器结构图。建议展示 CPU、CSR、MMU、Bus、DRAM、UART、CLINT、PLIC、BlockDevice 的调用关系。】
...
【图 7 占位：Shell fork/execve 用户程序执行链路图。】
...
1. **未适配 QEMU**：当前系统运行在自研 `cemu` 上，尚未证明能够直接在 QEMU `virt` 平台启动。
2. **未实现完整 Demand Paging**：当前支持 Sv39 和用户地址空间，但尚未实现 ELF 段懒加载、缺页修复、栈自动增长和换页机制。
3. **Shell 不支持管道**：当前 Shell 支持重定向和后台任务，但尚未实现 `cmd1 | cmd2` 形式的管道。
4. **UART 输入主要采用轮询方式**：当前可以完成终端输入输出，但尚未实现完整的 UART RX 中断输入路径。
5. **用户权限模型不完整**：当前文件系统没有实现多用户权限模型，也没有完整的用户组、权限位和访问控制。
6. **文件系统没有日志机制**：MiniFS v2 能持久化保存数据，但不支持日志和异常断电恢复。
7. **用户异常隔离测试仍需补充**：当前已统一运行底层和系统级测试，但尚缺少非法指令、用户态缺页等异常仅终止故障进程且不影响 Shell 的自动化测试。
