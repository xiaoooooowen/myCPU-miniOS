...
TEST_F(MmuTest, BareModePassThrough) {
  ...
  EXPECT_EQ(mmu.translate(0x80000000, Mmu::AccessType::Load), 0x80000000);
  EXPECT_EQ(mmu.translate(0x10000000, Mmu::AccessType::Load), 0x10000000);
  EXPECT_EQ(mmu.translate(0x00000000, Mmu::AccessType::Load), 0x00000000);
}
...
TEST_F(MmuTest, Sv39IdentityMapRead) {
  ...
  uint64_t paddr = mmu.translate(TEST_VA, Mmu::AccessType::Load);
...
TEST_F(MmuTest, Sv39IdentityMapReadWrite) {
  ...
  paddr = mmu.translate(TEST_VA, Mmu::AccessType::Store);
  EXPECT_EQ(paddr, TEST_PA);
}
...
TEST_F(MmuTest, Sv39IdentityMapExecute) {
  ...
  uint64_t paddr = mmu.translate(TEST_VA, Mmu::AccessType::Instruction);
  EXPECT_EQ(paddr, TEST_PA);
}
...
TEST_F(MmuTest, Sv39PageOffsetPreserved) {
  ...
  for (uint64_t off : {0x0ULL, 0x100ULL, 0xFFFULL, 0x3ABULL}) {
    ...
    uint64_t pa = mmu.translate(va, Mmu::AccessType::Load);
...
TEST_F(MmuTest, InvalidPTEPageFault) {
  ...
  EXPECT_THROW(mmu.translate(TEST_VA, Mmu::AccessType::Load), Exception);
}
...
TEST_F(MmuTest, StoreOnReadOnlyPageCausesPageFault) {
  ...
  EXPECT_THROW(mmu.translate(TEST_VA, Mmu::AccessType::Store), Exception);
}
...
TEST_F(MmuTest, FetchOnNoExecutePageCausesPageFault) {
  ...
  EXPECT_THROW(mmu.translate(TEST_VA, Mmu::AccessType::Instruction), Exception);
}
...
TEST_F(MmuTest, Sv39Megapage) {
  ...
  uint64_t paddr = mmu.translate(va, Mmu::AccessType::Load);
  EXPECT_EQ(paddr, pa);
}
